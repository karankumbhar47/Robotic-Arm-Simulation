...
class Planner(abc.ABC):
    ...
    def __init__(self, steps_per_seg=2, t_max=10.0, height=0.32):
        """
        Initialize the spline-based planner.

        Parameters:
            steps_per_seg (int): Number of steps per segment.
            t_max (float): Maximum time for the plan.
    ...
    def plot_quat(self, xnew):
        ...
        axes[1].plot(self.times, np.rad2deg(
            self.rot_spline(self.times, 1)), "x")
...
class PushPlanner(Planner):
    ...
    def _preprocess_poses(self, start_pose, pushstart_pose, pushend_pose):
        ...
        pos0 = np.float32(
            (pushstart_pose[0][0], pushstart_pose[0][1], 0.005))
